...
- (void)viewDidLoad {
    ...
    if(_gameLevel == WSGameLevelEasy){
    
    NSUInteger scoreCount1 = 0; // Just an example
    
    NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
    
    if (standardUserDefaults) {
        [standardUserDefaults setObject:[NSNumber numberWithInt:scoreCount1] forKey:@"Round1"];
    ...
    if(_gameLevel == WSGameLevelMedium){
    
    NSUInteger scoreCount2 = 0; // Just an example
    
    NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
     
     if (standardUserDefaults) {
         [standardUserDefaults setObject:[NSNumber numberWithInt:scoreCount2] forKey:@"Round2"];
         [standardUserDefaults synchronize];
     }
...
- (void)touchEndedAtPoint:(CGPoint)point {
    ...
    if (crossedOut_word){
        ...
        if (_gameLevel == WSGameLevelEasy ){
        _scoreCount1 += 1; // Just an example
          
          NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
          
          if (standardUserDefaults) {
              [standardUserDefaults setObject:[NSNumber numberWithInt:_scoreCount1] forKey:@"Round1"];
              [standardUserDefaults synchronize];
          }
            
            if(_scoreCount1 == 9){
                
               NSString *level = @"Complete1";
                
                NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
                         
                if (standardUserDefaults) {
                    [standardUserDefaults setObject:[NSString stringWithFormat:@"%@", level] forKey:@"Game1"];
                             [standardUserDefaults synchronize];
                         }
            }
        ...
        if(_gameLevel == WSGameLevelMedium){
            ...
            NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
                  ...
                  if (standardUserDefaults) {
                      [standardUserDefaults setObject:[NSNumber numberWithInt:_scoreCount2] forKey:@"Round1"];
                      [standardUserDefaults synchronize];
                  }
        ...
        NSLog(@"%lu", (unsigned long)_scoreCount1+_scoreCount2);
    }
